...
export const onRequestPost: PagesFunction<Env, 'committeeId'> = async ({
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO committee_control (committee_id, device_id, holder_name, heartbeat_at)
       VALUES (?1, ?2, ?3, ?4)
      ...
      .bind(committeeId, deviceId, name, serverNow, serverNow - CONTROL_TTL_MS, force ? 1 : 0)
      .run();
