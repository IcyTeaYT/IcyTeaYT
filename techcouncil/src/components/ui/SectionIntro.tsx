...
export function SectionIntro({ index, label, title, kicker, align = 'left', variant = 'rise', id, size = 'lg' }: Props) {
  ...
  return (
    <div className={align === 'center' ? 'mx-auto max-w-3xl text-center' : 'max-w-3xl'}>
      ...
      {variant === 'mask' && !reduce ? (
        // The wipe runs on an inner span. A heading clipped to zero width is
        // never "in view" for the observer (it stayed hidden on mobile), so
        // the unclipped h2 is what gets observed.
        <motion.h2
          id={id}
        ...
        >
          <motion.span
            className="block"
            variants={{
              hidden: { clipPath: 'inset(0 100% 0 0)', x: -24 },
              shown: { clipPath: 'inset(0 0% 0 0)', x: 0, transition: { duration: 1.1, ease: EASE_OUT, delay: 0.05 } },
            }}
          >
            {title}
          </motion.span>
